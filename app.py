...
# Initialize database
def init_db():
    conn = sqlite3.connect('techblog.db')
    cursor = conn.cursor()
...
<html>
<head>
    <title>File Upload - TechBlog</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background: #f5f5f5; }
        ...
        button { background: #1976d2; color: white; padding: 10px 20px; border: none; border-radius: 3px; }
        .error { color: red; }
        .success { color: green; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <p><small>Upload your code files, images, or documents to share with the community</small></p>
    </div>
</body>
</html>
'''
...
def ping():
    ...
    return render_template_string(ping_template)

ping_template = '''
<!DOCTYPE html>
<html>
<head>
    <title>Network Tools - TechBlog</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background: #f5f5f5; }
        .container { max-width: 800px; margin: 0 auto; background: white; padding: 20px; border-radius: 5px; }
        .nav { margin: 20px 0; }
        ...
        input[type="text"] { width: 60%; padding: 8px; border: 1px solid #ddd; border-radius: 3px; }
        button { background: #1976d2; color: white; padding: 8px 15px; border: none; border-radius: 3px; }
        .output { background: #000; color: #0f0; padding: 15px; margin: 15px 0; font-family: monospace; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <form method="POST">
            <input type="text" name="host" value="{{ host or '' }}" placeholder="Enter hostname or IP">
        ...
        <p><small>Test network connectivity to any host or IP address</small></p>
    </div>
</body>
</html>
'''

# Admin panel with IDOR vulnerability
@app.route('/admin')
def admin():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    <div class="container">
        ...
        <table>
            ...
            <tr>
                ...
                <td>{{ user[3] }}</td>
                <td>{{ user[4] }}</td>
                <td><a href="/profile?id={{ user[0] }}">View Profile</a></td>
            </tr>
            {% endfor %}
        </table>
    </div>
...
# Profile with IDOR vulnerability
@app.route('/profile')
def profile():
...
<html>
<head>
    <title>User Profile - TechBlog</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background: #f5f5f5; }
        .container { max-width: 600px; margin: 0 auto; background: white; padding: 20px; border-radius: 5px; }
        .nav { margin: 20px 0; }
        .nav a { color: #1976d2; text-decoration: none; }
        .profile-info { margin: 15px 0; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="profile-info">
            ...
            <strong>Password Hash:</strong> {{ user[2] }}
        </div>
        
        <p><small>User profile information and settings</small></p>
    </div>
</body>
</html>
    ''', user=user)
...
# Contact form with XXE
@app.route('/contact', methods=['GET', 'POST'])
def contact():
    if request.method == 'POST':
        if request.content_type == 'application/xml':
            # Process XML contact form
            try:
                ...
                return f"XML Contact received from {name} ({email}): {message}"
            except Exception as e:
            ...
            return f"Contact received from {name} ({email}): {message}"
...
<head>
    ...
    <style>
        ...
        button { background: #1976d2; color: white; padding: 10px 20px; border: none; border-radius: 3px; }
        .xml-example { background: #f0f0f0; padding: 10px; margin: 10px 0; font-family: monospace; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <h3>XML API</h3>
        <p>You can also submit via XML POST to this endpoint:</p>
...
&lt;/contact&gt;
        ...
        <p><small>We'd love to hear from you! Send us your feedback.</small></p>
    </div>
</body>
</html>
    ''')
...
if __name__ == '__main__':
    init_db()
    print("💻 TechBlog starting...")
    print("🌐 Server running at: http://localhost:5000")
    print("👥 User accounts available:")
    print("   sarah_dev / summer2023 (admin)")
    print("   mike_codes / password1")
    print("   alex_js / 123456")
    print("   jenny_react / qwerty")
    print("")
